...
defaultPackage.processor(
  new ListProcessor(docName, join(cwd, binDir), JSON.parse(dataRaw).data)
);
defaultPackage.processor(
  new CopyDataProcessor(
    absoluteOutputPath,
    join(cwd, binDir),
...
defaultPackage.config(function (
  ...
  tsParser.options.paths!['*'] = [
    relative(
      absoluteSourcePath,
      join(cwd, 'external/npm/node_modules/@types/*')
    ),
    relative(absoluteSourcePath, join(cwd, 'external/npm/node_modules/*')),
  ];

  for (const [moduleName, modulePath] of Object.entries<string>(
    JSON.parse(mappings)
  )) {
